...
#define Flash_Buf_Max 310

#define FLASH_USER_ADDRESS  0x0800F800L   //第63K空间中   
#define FLASH_NET_ADDRESS   0x0800FC00L //第64K空间中 

extern unsigned short bit16[30];

extern uint8_t Flash_Buf[Flash_Buf_Max];

extern void FlashIn_Erase_Page(uint32_t Start_Addr,uint8_t NbPage);

static uint8_t FlashIn_Read_Byte(__IO uint32_t Start_Addr);
extern void FlashIn_Write_NByte(__IO uint32_t Address,uint8_t* datas,uint8_t NByte);
extern void FlashIn_Read_NByte(uint32_t Start_Addr,uint16_t NByte);
extern uint32_t Get_UDID(void);
extern void bit8tobit16(unsigned char* bit8,unsigned short* bit16,unsigned char bit8len);
